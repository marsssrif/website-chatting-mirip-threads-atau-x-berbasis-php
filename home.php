        ...
        <div class="mid-col">
            <div class="header">
                ...
                if (isset($_GET['feed']) && $_GET['feed'] == 'ghost') {
                    ...
                    echo (isset($_SESSION['username']) ? htmlspecialchars($_SESSION['username']) . " - " : "") . "Home";
                }
                ?>
            </div>
            ...
            <?php if (isset($_SESSION['user_id'])): ?>
                <div class="post-form">
                    <form method="POST" action="" enctype="multipart/form-data" id="meowForm">
                        <textarea name="content" rows="3" placeholder="Apa yang sedang kamu pikirkan, Meow?"
                            required></textarea>
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div style="display: flex; align-items: center; gap: 12px;">
                                ...
                                <input type="file" name="post_img" id="post_img" accept="image/png, image/jpeg"
                                    class="input-file-hidden">

                                <label
                                    style="display: flex; align-items: center; gap: 6px; font-size: 14px; font-weight: 700; color: var(--text-muted); cursor: pointer; user-select: none;">
                                    <input type="checkbox" name="is_ghost" value="1"
            ...
            <?php foreach ($all_posts as $post): ?>
                <div class="feed-post" id="post-<?php echo $post['id']; ?>">
                    <?php if (isset($post['is_ghost']) && $post['is_ghost'] == 1): ?>
                        <div class="avatar"
                            style="display: flex; align-items: center; justify-content: center; background: var(--border-color); color: var(--text-muted); font-size: 22px;">
                            <i class="bi bi-ghost"></i>
                        </div>
                    <?php else: ?>
                        <a href="profile.php?username=<?php echo urlencode($post['username']); ?>">
                            <img src="uploads/avatars/<?php echo $post['profile_pic']; ?>" class="avatar" alt="ava"
                    ...
                    <div class="post-content" style="width: 100%;">
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            ...
                            <?php else: ?>
                                <a href="profile.php?username=<?php echo urlencode($post['username']); ?>"
                                    ...
                                    <h4 style="margin: 0; display: inline-block; cursor: pointer;">
                                        <?php echo htmlspecialchars($post['name']); ?>
                                    </h4>
                                    <span style="color: var(--text-muted); font-size: 14px; cursor: pointer;">
                                        @<?php echo htmlspecialchars($post['username']); ?></span>
                                </a>
                            <?php endif; ?>

                            <?php if (isset($_SESSION['user_id']) && $_SESSION['user_id'] == $post['user_id']): ?>
                                <a href="home.php?delete_id=<?php echo $post['id']; ?>"
                                    onclick="return confirm('Yakin ingin menghapus Meow ini?')"
                                    style="color: #ef4444; text-decoration: none; font-size: 12px; font-weight: 600;">🗑️
                                    Hapus</a>
                            <?php endif; ?>
                        </div>
                        ...
                        <a href="post_detail.php?id=<?php echo $post['id']; ?>"
                            ...
                            <?php if (!empty($post['post_image'])): ?>
                                ...
                                <div style="margin-top: 8px;">
                                    <a href="download.php?file=<?php echo urlencode($post['post_image']); ?>"
                                        class="download-link">📥 Download Gambar</a>
                                </div>
                            <?php endif; ?>
                        </a>
                        ...
                        <div class="post-actions">
                            <a href="home.php?like_id=<?php echo $post['id']; ?>#post-<?php echo $post['id']; ?>"
                                class="post-action-btn <?php echo $is_liked ? 'liked' : ''; ?>">
                                <?php echo $is_liked ? '❤️' : '🤍'; ?>
                            ...
                            <a href="home.php?save_id=<?php echo $post['id']; ?>#post-<?php echo $post['id']; ?>"
                                class="post-action-btn <?php echo $is_saved ? 'saved' : ''; ?>"
                                style="color: <?php echo $is_saved ? 'var(--primary)' : 'inherit'; ?>;">
                                <?php echo $is_saved ? '🔖' : '📑'; ?>
                                <span><?php echo $is_saved ? 'Tersimpan' : 'Simpan'; ?></span>
                            </a>
                        </div>
                    </div>
                </div>
